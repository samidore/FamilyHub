...
assert(meals.ingredients.length === 135 && meals.ingredients.filter((item) => item.visible).length === 132 && meals.recipes.length === 164, 'Meal Builder active counts are incorrect');
assert(meals.recipes.filter((item) => item.vegetableCentered).length === 24, 'Vegetable-centered Recipe count is not 24');
assert(meals.recipes.filter((item) => item.tags?.includes('iron-pan-braise')).length === 36, 'Iron-pan braise capability count is not 36');
...
assert(meals.ingredients.filter((item) => item.tags?.includes('easy-braise-addon')).length === 23, 'Easy-braise capability count is not 23');
