...
const dayTripData = JSON.parse(await readFile(new URL('../src/data/day-trips.json', import.meta.url), 'utf8')) as Array<{ name: string; city: string; state: string; driveMinutes: number | null }>;
const dayTripCount = dayTripData.length;
const within20DayTripCount = dayTripData.filter((trip) => trip.driveMinutes !== null && trip.driveMinutes <= 20).length;
...
const mcfaulDriveText = `${mcfaulData.city}, ${mcfaulData.state} · ${mcfaulData.driveMinutes === null ? '车程待核实' : `约 ${mcfaulData.driveMinutes} 分钟`}`;
...
test('Recipe without optional groups does not expose optional Checkout controls', async ({ page }) => {
  await page.goto('meal-builder/');
  await startMeal(page, ['chicken-breast', 'fried-tofu-puffs']);
  await selectRecipe(page, 'simple-pan-seared-chicken-breast');
  await page.locator('#meal-force-next').click();
  await page.locator('#meal-open-checkout').click();
  const checkoutCard = page.locator('[data-checkout-recipe="simple-pan-seared-chicken-breast"]');
...
test('household inventory keeps counted half-steps, presence-only values, and visible starter scope', async ({ page }) => {
  ...
  await expect(presence.locator('[data-stock-toggle]')).toHaveText('移除');
...
test('Recipe-scoped checkout consumes counted inventory while presence-only defaults to keep', async ({ page }) => {
  ...
  await selectRecipe(page, 'basic-egg-drop-soup');
  await selectRecipe(page, 'tomato-scrambled-eggs');
  await page.locator('#meal-force-next').click();
  await page.locator('#meal-open-checkout').click();

  const tomatoCard = page.locator('[data-checkout-recipe="tomato-scrambled-eggs"]');
  const eggsUsedUp = tomatoCard.locator('[data-actual-used-up-ingredient="eggs"]');
  ...
  const tomatoMinus = tomatoCard.locator('[data-actual-step-ingredient="tomato"][data-actual-step="-0.5"]');
  const tomatoPlus = tomatoCard.locator('[data-actual-step-ingredient="tomato"][data-actual-step="0.5"]');
  ...
  await expect(page.locator('#meal-builder-view')).toBeVisible();
  await page.locator('[data-step-target="inventory"]').click();
  await expect(page.locator('[data-inventory-item="tomato"] [data-inventory-value="tomato"]')).toHaveCount(0);
  await expect(page.locator('[data-inventory-item="eggs"] [data-stock-toggle]')).toHaveText('移除');
  await expect(page.locator('[data-inventory-item="noodles"] [data-stock-toggle]')).toHaveText('移除');
});
...
test('two pages in one browser context receive inventory and current-meal updates', async ({ browser }) => {
  ...
  await expect.poll(async () => first.locator('[data-inventory-item="chicken-breast"] [data-inventory-value="chicken-breast"]').count()).toBe(0);
