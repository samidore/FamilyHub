...
test('item authorName is normalized and invalid identity-like values are rejected', () => {
  const valid = normalizeNotebookState({ items: { i: item('i', '  呜哇  ') } });
  assert.equal(valid.items.i.authorName, '呜哇');
  const invalid = normalizeNotebookState({ items: { i: item('i', 'person@example.com') } });
  assert.deepEqual(invalid.items, {});
});

test('attribution snapshots only newly created items and never rewrites existing authors', () => {
