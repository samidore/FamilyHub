...
export type ServingIngredientId = 'rice' | 'noodles';
...
export interface MealFinishOption { id: string; labelZh: string; default: boolean; displayNameZh?: string; cookIngredientLines: string[]; steps: string[]; }
...
export interface MealRecipe {
  ...
  finishOptions?: MealFinishOption[];
  servingOptions?: ServingIngredientId[];
...
export const defaultMealState = (): MealState => ({
  ...
  recipeIngredientBindings: {}, selectedAddons: [], recipeFinishSelections: {}, recipeServingSelections: {}, ingredientFreshnessDates: {},
});
...
export function optionalGroupsForRecipe(recipe: MealRecipe | undefined, groups: MealOptionalGroup[] = []) {
  if (!recipe) return [];
  const map = optionalGroupMap(groups); const seen = new Set<string>();
  return (recipe.optionalGroupIds ?? []).map((id) => map.get(id)).filter((group): group is MealOptionalGroup => Boolean(group)).map((group) => ({
    ...group,
    ingredients: group.ingredients.filter((entry) => { if (seen.has(entry.ingredientId) || recipe.requirements.some((requirement) => requirement.anyOf.includes(entry.ingredientId))) return false; seen.add(entry.ingredientId); return true; }),
...
export function finishOptionsForRecipe(recipe: MealRecipe | undefined) { return recipe?.finishOptions ?? []; }
export function defaultFinishId(recipe: MealRecipe | undefined) { return finishOptionsForRecipe(recipe).find((finish) => finish.default)?.id ?? finishOptionsForRecipe(recipe)[0]?.id ?? null; }
export function finishForRecipe(recipe: MealRecipe | undefined, finishId: string | null | undefined) { return finishOptionsForRecipe(recipe).find((finish) => finish.id === finishId) ?? finishOptionsForRecipe(recipe).find((finish) => finish.default) ?? finishOptionsForRecipe(recipe)[0] ?? null; }
export function resolvedRecipeName(recipe: MealRecipe & { nameZh?: string } | undefined, finishId?: string | null) { const finish = finishForRecipe(recipe, finishId); return finish?.displayNameZh ?? recipe?.nameZh ?? recipe?.id ?? ''; }
export function servingOptionsForRecipe(recipe: MealRecipe | undefined, availableIngredientIds?: Set<string> | string[], selected?: ServingIngredientId | null) {
  const available = asSet(availableIngredientIds); return (recipe?.servingOptions ?? []).filter((id) => available.has(id) || id === selected);
}
...
export function aggregateMeal(selected: MealRecipe[], context: { bindings?: Record<string, (string | null | undefined)[]>; recipeIngredientBindings?: Record<string, (string | null | undefined)[]>; selectedAddons?: SelectedAddon[]; recipeServingSelections?: Record<string, ServingIngredientId>; optionalGroups?: MealOptionalGroup[]; availableIngredientIds?: string[]; ingredients?: MealIngredient[] | Record<string, MealIngredient>; } = {}): MealTotals {
  ...
  const servingSelections = context.recipeServingSelections ?? {};
  return selected.reduce<MealTotals>((total, recipe) => { const binding = bindings[recipe.id] ?? (available.size ? bindRecipeIngredients(recipe, available) : []); const coverage = resolveRecipeChildCoverage(recipe, binding, context.ingredients); const optional = selectedOptionalContribution(recipe, optionState, context.optionalGroups ?? [], context.ingredients); const serving = servingSelections[recipe.id]; const servingContribution = serving && (recipe.servingOptions ?? []).includes(serving) ? 1 : 0; total.protein += recipe.contribution.protein + optional.contribution.protein; total.vegetable += recipe.contribution.vegetable + optional.contribution.vegetable; total.staple += recipe.contribution.staple + optional.contribution.staple + servingContribution; total.childProtein ||= coverage.protein || optional.childProtein; total.childVegetable ||= coverage.vegetable || optional.childVegetable; return total; }, { protein: 0, vegetable: 0, staple: 0, childProtein: false, childVegetable: false });
}
export const aggregateSelection = (recipes: MealRecipe[], state: MealState, ingredients?: MealIngredient[] | Record<string, MealIngredient>, optionalGroups: MealOptionalGroup[] = []) => aggregateMeal(recipes.filter((recipe) => state.selectedRecipeIds.includes(recipe.id)), { recipeIngredientBindings: state.recipeIngredientBindings, selectedAddons: state.selectedAddons, recipeServingSelections: state.recipeServingSelections, optionalGroups, availableIngredientIds: state.availableIngredientIds, ingredients });
...
export function reconcileMealState(input: Partial<MealState>, recipes: MealRecipe[], _ingredients: MealIngredient[] | Record<string, MealIngredient> = [], optionalGroups: MealOptionalGroup[] = []): MealState {
  const state: MealState = { ...defaultMealState(), ...input, recipeIngredientBindings: { ...(input.recipeIngredientBindings ?? {}) }, selectedAddons: [...(input.selectedAddons ?? [])], recipeFinishSelections: { ...(input.recipeFinishSelections ?? {}) }, recipeServingSelections: { ...(input.recipeServingSelections ?? {}) }, ingredientFreshnessDates: { ...(input.ingredientFreshnessDates ?? {}) } }; const available = asSet(state.availableIngredientIds); const keptRecipes: string[] = []; const bindings: Record<string, string[]> = {};
  const finishSelections: Record<string, string> = {}; const servingSelections: Record<string, ServingIngredientId> = {};
  for (const id of [...new Set(state.selectedRecipeIds)]) { const recipe = recipes.find((item) => item.id === id); if (!recipe) continue; const existing = state.recipeIngredientBindings[id] ?? []; const binding = recipe.requirements.map((requirement, index) => { const prior = existing[index]; if (prior && requirement.anyOf.includes(prior)) return prior; return requirement.anyOf.find((ingredientId) => available.has(ingredientId)) ?? null; }); if (binding.some((value) => value === null)) continue; keptRecipes.push(id); bindings[id] = binding.filter((value): value is string => Boolean(value)); const finishId = state.recipeFinishSelections[id]; const defaultId = defaultFinishId(recipe); if (defaultId) finishSelections[id] = finishOptionsForRecipe(recipe).some((finish) => finish.id === finishId) ? finishId : defaultId; const serving = state.recipeServingSelections[id]; if (serving && servingOptionsForRecipe(recipe, available).includes(serving)) servingSelections[id] = serving; }
  const kept = new Set(keptRecipes); const recipeMap = new Map(recipes.map((recipe) => [recipe.id, recipe])); const selectedAddons = (state.selectedAddons ?? []).flatMap((addon) => { if (!kept.has(addon.mainRecipeId)) return []; const recipe = recipeMap.get(addon.mainRecipeId); const entry = optionalIngredientForRecipe(recipe, addon.addonType, addon.ingredientId, optionalGroups); if (entry) return [addon]; const firstGroup = optionalGroupsForRecipe(recipe, optionalGroups).find((group) => group.ingredients.some((candidate) => candidate.ingredientId === addon.ingredientId)); return firstGroup ? [{ ...addon, addonType: firstGroup.id }] : []; }); return { ...state, availableIngredientIds: [...available], selectedRecipeIds: keptRecipes, recipeIngredientBindings: bindings, selectedAddons, recipeFinishSelections: finishSelections, recipeServingSelections: servingSelections };
}

export function checkoutUnitsForSelection(recipes: MealRecipe[], state: MealState): Record<string, number> { const totals: Record<string, number> = {}; for (const recipe of recipes.filter((item) => state.selectedRecipeIds.includes(item.id))) { for (const id of [...new Set(state.recipeIngredientBindings[recipe.id] ?? [])]) totals[id] = (totals[id] ?? 0) + (recipe.checkoutUnits?.[id] ?? 1); const serving = state.recipeServingSelections[recipe.id]; if (serving && (recipe.servingOptions ?? []).includes(serving)) totals[serving] = (totals[serving] ?? 0) + 1; } return totals; }
