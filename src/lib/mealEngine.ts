...
function resolveCoverage(recipe: MealRecipe, slot: 'protein' | 'vegetable', binding: (string | null | undefined)[] = [], ingredients?: MealIngredient[] | Record<string, MealIngredient>) {
  if (recipeChildAll(recipe) && recipe.contribution[slot] > 0) return true; const relevant = relevantBoundIds(recipe, slot, binding); if (relevant.some((id) => ingredientChildEaten(id, ingredients))) return true; const declared = recipe.childCoverage?.[slot]; if (declared === true) return true; if (declared === false || declared !== 'ingredient-dependent') return false; const map = ingredientMap(ingredients); return relevant.some((id) => Boolean(id && (recipe.ingredientChildCoverage?.[id] === true || map.get(id)?.childCoverage?.vegetable === true)));
}
...
export function rankCandidates(recipes: MealRecipe[], state: MealState, ingredients: MealIngredient[] | Record<string, MealIngredient> = [], draftBindings: Record<string, (string | null | undefined)[]> = {}, recentRecipeIds: string[][] = [], today = calendarDateKey(), optionalGroups: MealOptionalGroup[] = []) {
  ...
  return recipes.filter((recipe) => { if (selectedIds.has(recipe.id)) return false; const value = measures(recipe); if (!isFeasible(recipe, available, value.binding)) return false; return value.childSolved > 0 || value.normalGaps > 0 || value.potentialChild > 0 || value.potentialGaps > 0; }).sort((a, b) => { const x = measures(a); const y = measures(b); const staleOrder = Number(y.stalePriority) - Number(x.stalePriority); const staleAgeOrder = x.stalePriority && y.stalePriority ? y.oldestAgeDays - x.oldestAgeDays : 0; return staleOrder || staleAgeOrder || y.childSolved - x.childSolved || Number(y.withinProteinTolerance) - Number(x.withinProteinTolerance) || y.normalGaps - x.normalGaps || x.overage - y.overage || y.potentialChild - x.potentialChild || y.potentialGaps - x.potentialGaps || recentRecipePenalty(a.id, recentRecipeIds) - recentRecipePenalty(b.id, recentRecipeIds) || b.fitScore - a.fitScore || x.time.rank - y.time.rank || a.order - b.order; });
}
