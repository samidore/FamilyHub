...
export function rankCandidates(recipes: MealRecipe[], state: MealState, ingredients: MealIngredient[] | Record<string, MealIngredient> = [], draftBindings: Record<string, (string | null | undefined)[]> = {}, recentRecipeIds: string[][] = [], today = calendarDateKey(), optionalGroups: MealOptionalGroup[] = []) {
  ...
  return recipes.filter((recipe) => { if (selectedIds.has(recipe.id)) return false; const value = measures(recipe); if (!isFeasible(recipe, available, value.binding)) return false; return value.childSolved > 0 || value.normalGaps > 0 || value.potentialChild > 0 || value.potentialGaps > 0; }).sort((a, b) => { const x = measures(a); const y = measures(b); const reuseOrder = Number(x.reusesSelectedIngredient) - Number(y.reusesSelectedIngredient); const staleOrder = Number(y.stalePriority) - Number(x.stalePriority); const staleAgeOrder = x.stalePriority && y.stalePriority ? y.oldestAgeDays - x.oldestAgeDays : 0; return reuseOrder || staleOrder || staleAgeOrder || y.childSolved - x.childSolved || Number(y.withinProteinTolerance) - Number(x.withinProteinTolerance) || y.normalGaps - x.normalGaps || x.overage - y.overage || y.potentialChild - x.potentialChild || y.potentialGaps - x.potentialGaps || recentRecipePenalty(a.id, recentRecipeIds) - recentRecipePenalty(b.id, recentRecipeIds) || b.fitScore - a.fitScore || x.time.rank - y.time.rank || a.order - b.order; });
}
