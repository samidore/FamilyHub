...
export function normalizeDiscardedStock(value: unknown): DiscardedStocks {
  ...
  for (const [id, raw] of Object.entries(value)) {
    ...
    const batchKey = raw.batchKey === undefined ? undefined : typeof raw.batchKey === 'string' && (DATE_KEY_PATTERN.test(raw.batchKey)) ? raw.batchKey : null;
...
export function normalizeHouseholdState(value: unknown, ingredients?: MealIngredient[] | Record<string, MealIngredient>): HouseholdState {
  ...
  return { inventory, inventoryBatches, freezerInventory, thawingItems, discardedStock, currentMeal, pendingCheckoutMeals, activeStep: currentMeal ? activeStep : 'inventory', recentMeals };
}
...
export function adjustInventoryBatch(state: HouseholdState, ingredientId: string, batchKey: string, delta: number, ingredients?: MealIngredient[] | Record<string, MealIngredient>): HouseholdState {
  if (!isFifoIngredient(ingredientId, ingredients) || !DATE_KEY_PATTERN.test(batchKey) || !isStepAligned(delta)) return state;
  const current = state.inventoryBatches[ingredientId]?.[batchKey] ?? 0;
  if (delta < 0 && (current + delta < 0 || (state.inventory[ingredientId] as number ?? 0) + delta < (queuedReservationUnits(state, ingredients)[ingredientId] ?? 0))) return state;
  const nextBatches = { ...(state.inventoryBatches[ingredientId] ?? {}) }; const nextQuantity = roundCountedInventoryValue(current + delta);
  if (nextQuantity > 0) nextBatches[batchKey] = nextQuantity; else delete nextBatches[batchKey];
...
export function adjustFrozenAggregate(state: HouseholdState, ingredientId: string, delta: number, ingredients?: MealIngredient[] | Record<string, MealIngredient>): HouseholdState {
  ...
  if (delta < 0 && target === 'inventory' && (state.inventory[ingredientId] as number ?? 0) + delta < (queuedReservationUnits(state, ingredients)[ingredientId] ?? 0)) return state;
  return { ...state, [target]: adjustInventoryItem(state[target], ingredientId, delta) } as HouseholdState;
}
export function addStock(state: HouseholdState, ingredientId: string, storage: 'inventory' | 'freezer', quantity: number, ingredients?: MealIngredient[] | Record<string, MealIngredient>, batchKey = calendarDateKey()): HouseholdState {
  ...
  if (tracking === 'presence-only') { const target = storage === 'freezer' && item.freezerBehavior !== 'direct' ? 'freezerInventory' : 'inventory'; return { ...state, [target]: toggleInventoryItem(state[target], ingredientId, tracking, true) } as HouseholdState; }
  ...
  const batches = target === 'inventory' && item.inventoryFreshness === 'fifo' ? { ...state.inventoryBatches, [ingredientId]: addInventoryBatch(state.inventoryBatches[ingredientId] ?? {}, batchKey, quantity) } : state.inventoryBatches;
  return { ...next, inventoryBatches: batches };
}
export function discardStock(state: HouseholdState, ingredientId: string, storage: 'inventory' | 'freezer', now = Date.now(), batchKey?: string, ingredients?: MealIngredient[] | Record<string, MealIngredient>): HouseholdState {
  const item = ingredientRecord(ingredientId, ingredients); const source = storage === 'freezer' && item?.freezerBehavior === 'thaw-required' ? state.freezerInventory[ingredientId] : state.inventory[ingredientId]; const quantity = typeof source === 'number' ? (batchKey ? (storage === 'freezer' ? 0 : state.inventoryBatches[ingredientId]?.[batchKey]) ?? 0 : source) : undefined; const presence = source === true;
  ...
  const batchStore = state.inventoryBatches; if (batchKey && storage === 'inventory' && batchStore[ingredientId]) { const batches = { ...batchStore[ingredientId] }; delete batches[batchKey]; next = { ...next, inventoryBatches: { ...state.inventoryBatches, [ingredientId]: batches } }; }
...
export function undoDiscard(state: HouseholdState, recordId: string, now = Date.now(), ingredients?: MealIngredient[] | Record<string, MealIngredient>): HouseholdState {
  ...
  if (record.batchKey && record.quantity && record.storage === 'inventory') { const batches = next.inventoryBatches; const updated = { ...(batches[record.ingredientId] ?? {}), [record.batchKey]: roundCountedInventoryValue((batches[record.ingredientId]?.[record.batchKey] ?? 0) + record.quantity) }; next = { ...next, inventoryBatches: { ...batches, [record.ingredientId]: updated } }; }
