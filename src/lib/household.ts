...
export interface CurrentMeal {
  ...
  /** Shared checkout draft; values are presence toggles or positive half steps. */
...
export function normalizeCheckoutConsumption(value: unknown, ingredients?: MealIngredient[] | Record<string, MealIngredient>): CheckoutConsumption {
  ...
  for (const [id, raw] of Object.entries(value)) {
    ...
    else if (typeof raw === 'number' && Number.isFinite(raw) && raw >= 0 && Math.abs(raw * 2 - Math.round(raw * 2)) < 1e-9) normalized[id] = Math.round(raw * 2) / 2;
  }
  return normalized;
}
