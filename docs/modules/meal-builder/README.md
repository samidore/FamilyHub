...
- [`grocery-recording.md`](grocery-recording.md): triggered grocery-recording workflow, live Ingredient matching, receipt reconciliation, and final JSON handoff.
...
Read only what the change needs: `behavior.md` for UI or meal logic, `data-model.md` for YAML/records/indexes/loaders, `inventory-import.md` for Chat-assisted bulk inventory entry, `grocery-recording.md` only when entering grocery recording mode, and `firebase.md` for shared state.
