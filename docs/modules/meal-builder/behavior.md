...
- Recipe availability is evaluated from `ingredients[].availability: required`, any matching `one_of` option, and `pantry_core: assumed`. A Starter ingredient is available for this meal; it is not mandatory to consume.
