...
- In Recipes, a candidate or selected Recipe whose current binding uses FIFO stock older than the same strict `>3`-day threshold shows a compact hourglass badge labeled `临期`. The badge does not show age, Ingredient names, or other freshness explanation; Cook View and Checkout do not display it.
