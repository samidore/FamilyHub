...
2. **Recipes** — users set Protein and Vegetable planning targets, optional Staple, time preference, and Child mode; select available ingredients; then review ranked candidates. Every candidate exposes a compact `Required / One of / Optional` composition summary before selection. The first `选择这道菜` tap opens an inline local draft instead of selecting the Recipe; fixed requirements are displayed, `one_of` requirements are single-select, and each referenced optional group renders as its own multi-select section such as `加点油水`, `加番茄`, `顺手焖`, or `汤里加`. An Ingredient already used by a hard or `one_of` requirement is not offered again as an optional. Recipes with `finish_options` also show one Finish choice with the default preselected; `serving_options` show one `none`/`rice`/`noodles` choice from the frozen available snapshot. Only `确认选择` commits the Recipe plus its bindings, Finish, serving, and planned optionals. A selected Recipe shows its planned composition and can reopen the same draft from `编辑`. The complete available-ingredient filter is wrapped in one overall fold that defaults closed whenever the Recipes view is entered; its compact summary keeps the selected-count visible, while opening it preserves the existing per-section folds and bulk controls. A new meal snapshots inventory **after** queued main-ingredient reservations, so an already-reserved counted main ingredient cannot be planned again unless another unit remains.
3. **Cook** — selected Recipes and their child-serving/adult-finish notes render in Cook View. Every selected Recipe has an always-visible `本次` block directly under the Recipe heading: current hard/`one_of` bindings are visible without opening anything, the resolved Finish name and Finish cook additions are shown, a selected serving is shown, every referenced optional group remains visible, selected optional members are emphasized, and an empty optional group explicitly says `不加`. Long cooking instructions may remain collapsed separately. The household can either continue to Checkout or choose `排队结算`.
4. **Checkout** — each selected Recipe gets its own Actual-composition card. The card starts from Plan, but the cook may switch a `one_of` choice, remove a planned optional, add an unplanned optional from current live inventory, switch/remove a planned serving, and adjust actual consumed quantities. Finish remains display-only and never creates an Actual inventory row. If earlier meals are queued and the current meal is already cooking, Checkout presents the queued meals plus the current meal together and the final confirmation aggregates all their Actual consumption. A queued-meal warning also exposes a direct `结算` action: when the next meal is still in Inventory/Recipes, that action checks out only the queued meals and preserves the unfinished current meal and its step. Confirm and commit atomically. Counted FIFO Ingredients are deducted from the oldest batch first in the same transaction. A stale meal or invalid quantity does not partially mutate inventory or batch metadata.
...
- Every candidate Recipe card shows a compact composition summary before selection: fixed required Ingredients, `one_of` alternatives, and referenced optional-group labels. The collapsed summary does not enumerate every optional member; a missing concept displays `—`.
- Every candidate Recipe card also summarizes available Finish labels and serving choices. The default Finish resolves to the base `name_zh`; non-default Finish display names are local to that Recipe.
- The first `选择这道菜` tap opens an inline Plan draft only. It does not add the Recipe to `selectedRecipeIds`, change completion totals, affect selected-Ingredient reuse ranking, or persist shared meal state.
- Fixed hard requirements are shown but cannot be deselected.
- Each `one_of` requirement is one section with exactly one active binding rendered as direct pressed/unpressed controls.
...
- Overlapping central groups are deduped by referenced group order. Hard and `one_of` requirement Ingredients are hidden from optional sections.
- Finish is single-select and always has exactly one default when present. Finish additions are Cook View guidance only and do not affect Plan totals or inventory.
- Serving is single-select. `rice` or `noodles` adds one Staple in Plan when selected; no serving remains no Staple contribution from this feature. Only a currently available serving is offered, with an already-planned serving retained while editing.
- Plan optional choices are limited to the meal's frozen availability snapshot, except an already-selected choice remains visible while editing a selected Recipe so it can be deselected.
- A hard/`one_of` bound Ingredient is never shown again as an optional choice for the same Recipe.
- Plan `one_of` and optional controls update their draft pressed state synchronously on tap. Draft changes do not affect effective totals, candidate ranking, or shared persistence.
- `取消` discards the local draft. For a new candidate this leaves the Recipe unselected; for an existing selected Recipe it restores the persisted Plan without changes.
- `确认选择` atomically commits a new Recipe together with its current bindings and planned optionals. A selected Recipe exposes `编辑`; `确认修改` atomically replaces that Recipe's bindings and planned optionals without removing and re-adding the Recipe. Only after confirmation do derived totals, candidate re-ranking, and shared persistence reflect the Plan.
...
+ selected serving: +1 Staple
```

Finish selection is intentionally absent from this calculation. It changes the resolved display name and Cook guidance only.
...
- Actual starts from the planned serving when one was selected.
- The cook may switch `one_of` to another valid live-stock alternative.
- The cook may remove a planned optional.
- The cook may switch to another available serving or remove the serving. Serving consumption is one counted unit and is aggregated with the other Actual rows.
...
- **Within each reuse group, freshness priority is ordered by Ingredient role before age.** A qualifying FIFO Ingredient used by a fixed hard requirement ranks above one used by the current `one_of` binding, which ranks above one merely available through a referenced optional group. Role outranks age, so an older optional never jumps ahead of a younger qualifying hard or `one_of` Ingredient. Within the same role, the oldest qualifying Ingredient ranks first. Fixed hard and `one_of` scopes use the current binding; optional scope includes available referenced group members even before selection. A FIFO Ingredient qualifies only when its frozen oldest-stock age is strictly greater than its own `freshness_priority_days`; an unavailable optional does not count. Only after freshness role and age does the normal correctness/repetition/fit/time ordering apply within that reuse group.
- In Recipes, every qualifying Ingredient across the current hard/`one_of` binding plus available referenced optional members is still shown in the existing freshness-signal position as a compact red outlined badge with the hourglass icon, Ingredient name, and age in days from the current meal's frozen oldest-stock date (for example, `番茄 8天`). Available optional members render even before the user selects them; multiple qualifying Ingredients render as separate badges. The visible badge does not use the word `临期`. Cook View and Checkout do not display freshness badges.
