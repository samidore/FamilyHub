...
- **Within each reuse group, freshness-priority candidates are placed first.** A candidate qualifies when its current binding uses a FIFO Ingredient older than that Ingredient's strict `freshness_priority_days` threshold. Among qualifying candidates, the oldest qualifying bound Ingredient ranks first. Only after freshness does the normal correctness/repetition/fit/time ordering apply within that reuse group.
- In Recipes, every qualifying bound Ingredient on a candidate or selected Recipe is shown in its existing freshness-signal position as a compact red outlined badge with the hourglass icon, Ingredient name, and age in days from the current meal's frozen oldest-stock date (for example, `豆腐 8天`). The visible badge does not use the word `临期`; multiple qualifying bound Ingredients render as separate badges. Cook View and Checkout do not display freshness badges.
