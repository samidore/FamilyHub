...
- `排队结算，开下一顿` moves the cooking meal into `pendingCheckoutMeals` and immediately creates the next selectable meal. A previous uncompleted checkout therefore never blocks planning the next meal.
...
- A visible queue warning is present throughout Meal Builder whenever at least one meal is queued, regardless of whether the active step is Inventory, Recipes, Cook, or Checkout. It reports queued-meal count and total counted main-ingredient units reserved.
- Queue order is preserved. When the current meal enters Checkout, all queued meals plus the current meal enter the same checkout session. Final totals are validated against current live inventory and committed once; either every meal is completed and the queue clears, or nothing is deducted.
