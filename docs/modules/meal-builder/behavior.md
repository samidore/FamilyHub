...
The Inventory step has internal `库存`, `冷冻`, and `化冻 N` views. `冷冻` manages freezer stock; `化冻` starts thawing stocked thaw-required Ingredients and manages current thaw jobs. Direct freezer Ingredients reuse ordinary inventory; thaw-required reserves enter ordinary inventory only after manual or automatic thaw completion. Manual `进入库存` transfers one unit per tap (or the remaining 0.5 unit), while automatic due completion transfers the full remaining thaw quantity.
