...
The Inventory step has internal `库存`, `冷冻`, and `化冻中 N` views. Direct freezer Ingredients reuse ordinary inventory; thaw-required reserves enter ordinary inventory only after manual or automatic thaw completion. Manual `进入库存` transfers one unit per tap (or the remaining 0.5 unit), while automatic due completion transfers the full remaining thaw quantity.
