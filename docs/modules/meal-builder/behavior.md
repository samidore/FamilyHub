...
The Inventory step has exactly two views: `入库` and `库存`. `入库` adds household stock and shows current amounts; `库存` manages refrigerated, frozen, and thawing lifecycle state together. Direct freezer Ingredients reuse ordinary inventory; thaw-required reserves enter ordinary inventory only after manual or automatic thaw completion. Manual `进入冷藏` transfers one unit per tap (or the remaining 0.5 unit), while automatic due completion transfers the full remaining thaw quantity.
...
Step 1 has exactly two tabs: `入库` and `库存`. `入库` shows current stock and adds one half-unit using one non-future local batch date. `库存` hides empty Ingredients and combines refrigerated, frozen, and thawing states under each Ingredient. Frozen counted batches are dated when known; legacy frozen aggregates remain `日期未知`. Thaw is batch-specific, lasts 36 hours, and completes into refrigerated stock; cancellation returns to the source batch. Every displayed batch can be discarded immediately and restored by any connected device for five minutes. These writes never rewrite the current-meal snapshot.
